...
class Matrix {
  ...
  void randomize() {
    for (int i = 0; i < matrix.length; i++) {
      for (int j = 0; j < matrix[i].length; j++) {
        matrix[i][j] = (float) Math.random() - 0.5f;
      }
    }
  }
  ...
  static Matrix randomize(int rows, int cols) {
    ...
    for (int i = 0; i < r.matrix.length; i++) {
      for (int j = 0; j < r.matrix[i].length; j++) {
        r.matrix[i][j] = (float) Math.random() - 0.5f;
      }
    }
    return r;
  }
  ...
  static float[][] sum(float[][] f, float[][] m) {
  ...
  static Matrix sum(Matrix m, Matrix f) {
    Matrix r = new Matrix(m.rows, m.cols);
    r.setVals(Matrix.sum(m.getVals(), f.getVals()));
    return r;
  }
  ...
  static float[][] diff(float[][] m, float[][] f) {
    float[][] c = new float[f.length][f[0].length];
    if (f.length == m.length && f[0].length == m[0].length) {
      for (int i = 0; i < m.length; i++) {
        for (int j = 0; j < m[i].length; j++) {
      ...
      System.out.println("ERROR: SUBTRACTION INCOMPATIBLE");
    }
    return c;
  }

  static Matrix diff(Matrix m, Matrix f) {
    Matrix r = new Matrix(m.rows, m.cols);
    r.setVals(Matrix.diff(m.getVals(), f.getVals()));
    return r;
  }
  /*********************************************************************************************************************/
  ...
  static Matrix multiply(Matrix m, float a) {
    Matrix result = new Matrix(m.rows, m.cols);
    result.setVals(m.matrix);
  ...
  float[][] multiply(float[][] b) {
    ...
    if (a[0].length != b.length) {
      System.out.println("ERROR MATRIX MULTIPLICATION INCOMPATIBLE");
      return a;
    }
  ...
  Matrix hadamard(Matrix f) {
    ...
    if (f.matrix.length == matrix.length && f.matrix[0].length == matrix[0].length) {
      ...
      System.out.println("ERROR HADAMARD INCOMPATIBLE");
    }
    return result;
  }
  ...
  static Matrix multiply(Matrix m, Matrix f) {
    Matrix result = new Matrix(m.rows, f.cols);
    result.setVals(Matrix.multiply(m.matrix, f.matrix));
    return result;
  }
  ...
  static float[][] transposeArr(Matrix m) {
    ...
    if (a.length == b.length && a[0].length == b[0].length) {
      for (int i = 0; i < b.length; i++) {
        for (int j = 0; j < b[i].length; j++) {
          c[i][j] = (float) Math.pow(a[i][j] - b[i][j], 2) * 0.5f;
        }
      }
    } else {
      System.out.println("ERROR: ERROR CALCULATION INCOMPATIBLE");
    }
    return c;
  }
