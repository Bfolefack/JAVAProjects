...
public class Zoomer {
    ...
    public static void initialize(float _s, NBody p) {
        sketch = p;
        xPan = sketch.width / 2;
    ...
    public static void pushZoom() {
        if (initialized) {
            NBody.truMouseX = (int) ((sketch.mouseX + xPan - sketch.width / 2) / scale);
            NBody.truMouseY = (int) ((sketch.mouseY + yPan - sketch.height / 2) / scale);
            sketch.pushMatrix();
