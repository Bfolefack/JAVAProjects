...
public class Zoomer {
    ...
    public static void initialize(float _s, AStar p) {
        sketch = p;
        xPan = sketch.width / 2;
    ...
    public static void pushZoom() {
        if (initialized) {
            AStar.truMouseX = (int) ((sketch.mouseX + xPan - sketch.width / 2) / scale);
            AStar.truMouseY = (int) ((sketch.mouseY + yPan - sketch.height / 2) / scale);
            sketch.pushMatrix();
