...
public class Zoomer {
    ...
    public static void initialize(float _s, Lenia p) {
        sketch = p;
        xPan = sketch.width / 2;
    ...
    public static void pushZoom() {
        if (initialized) {
            Lenia.truMouseX = (int) ((sketch.mouseX + xPan - sketch.width / 2) / scale);
            Lenia.truMouseY = (int) ((sketch.mouseY + yPan - sketch.height / 2) / scale);
            sketch.pushMatrix();
