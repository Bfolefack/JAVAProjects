...
public class Genome implements Serializable {
    ...
    public Genome(int in, int out, boolean reset, GeneLibrary lib) {
        library = lib;
        resetAfterRun = reset;
    ...
    public Genome(int in, int out, boolean reset, int lay, GeneLibrary lib) {
        library = lib;
        resetAfterRun = reset;
    ...
    public Genome(Genome genome) {
        ...
        for (Node node : genome.nodes.values()) {
            nodes.put(node.idNum, new Node(node.idNum, node.xPos));
        }
        for (ConnectionGene connection : genome.connections.values()) {
            connections.put(library.getConnectionInnovation(connection.in.idNum, connection.out.idNum), connection.copy());
        }
        generateNetwork();
    }
    ...
    public double[] feedForward(double[] inputValues) {
        if(inputValues.length != inputs){
            System.out.println("Inputs don't match");
            System.out.println("Expected: " + inputs + " Got: " + inputValues.length);
        }
        if (resetAfterRun) {
            clear();
        } else {
    ...
    public void addNode() {
        ...
        Integer idNum = library.getNodeID(og.in.idNum,og.out.idNum);
        Node newNode;
    ...
    public void addConnection() {
        ...
        while (true) {
            ...
            if (inNode.xPos == outNode.xPos || inNode.xPos >= layers - 1 || outNode.xPos <= 0) {
                ...
                innovation = library.isInnovation(inNode.idNum, outNode.idNum);
                innovationNum = library.getConnectionInnovation(inNode.idNum, outNode.idNum);
        ...
        if (innovation) {
            ConnectionGene newConnection = library.createConnectionGene(inNode, outNode, Math.random() * 4 - 2);
            int connectionID = library.getConnectionInnovation(inNode.idNum, outNode.idNum);
            connections.put(connectionID, newConnection);
        } else {
            connections.put(library.getConnectionInnovation(inNode.idNum, outNode.idNum),
    ...
    public void addConnection(int in, int out, double w) {
        ...
        if (innovation) {
            ConnectionGene newConnection = library.createConnectionGene(inNode, outNode, w);
            int connectionID = library.getConnectionInnovation(inNode.idNum, outNode.idNum);
            connections.put(connectionID, newConnection);
        } else {
            connections.put(library.getConnectionInnovation(inNode.idNum, outNode.idNum),
    ...
    public static Genome crossover(Genome top, Genome bottom) {
        if(top.library != bottom.library) {
            System.out.println("ERROR: CROSSOVER: LIBRARIES DO NOT MATCH");
            Thread.dumpStack();
            return null;
        }
        Genome child = new Genome(top.inputs, top.outputs, top.resetAfterRun, top.layers, top.library);
        child.clear();
